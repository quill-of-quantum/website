...
TXT_FILE = os.path.join(BASE_DIR, "number.txt")
...
daily.to_csv(os.path.join(BASE_DIR, "daily_usage.csv"), float_format='%.3f', encoding='utf-8-sig')
...
forecast_csv_path = os.path.join(BASE_DIR, "forecast_usage.csv")
...
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_forecast.svg"), format="svg")
plt.close()
...
format_time_axis(ax)
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_cumulative.svg"), format="svg")
plt.close()
...
if not weather_df.empty:
    ...
    # ✅ 新增：绘制室内温度（绿色插值曲线）
    temp_inside_path = "/home/bbdwz/projects/website/weather/temperature_inside.csv"
...
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_hourly.svg"), format="svg")
plt.close()
...
format_time_axis(ax1, "day")
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_daily.svg"), format="svg")
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_pattern.svg"), format="svg")
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_pattern_polar.svg"), format="svg")
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, "usage_heatmap.svg"), format="svg")
plt.close()
